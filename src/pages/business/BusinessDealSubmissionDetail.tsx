...
const BusinessDealSubmissionDetail: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const { data } = await supabase
        ...
        .select('id, tiktok_video_url, tiktok_video_id, status, current_views, current_likes, created_at, creator_id, message')
        .eq('id', applicationId)
        .maybeSingle();
      if (data) {
        setApplication(data);
  ...
  const tiktokEmbedUrl = application.tiktok_video_id
    ? `https://www.tiktok.com/embed/v2/${application.tiktok_video_id}`
    : null;
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-10">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <Badge variant="outline" className={statusStyles[application.status] || 'bg-muted text-muted-foreground'}>
          {statusLabels[application.status] || application.status}
        </Badge>
      </div>

      {/* Stats */}
      {(application.current_views || application.current_likes) ? (
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div className="rounded-[20px] p-4" style={cardStyle}>
            ...
            <p className="text-xl font-bold text-foreground mt-1">
              {(application.current_views || 0).toLocaleString()}
            </p>
          </div>
          <div className="rounded-[20px] p-4" style={cardStyle}>
            <div className="flex items-center gap-2">
            ...
            <p className="text-xl font-bold text-foreground mt-1">
              {(application.current_likes || 0).toLocaleString()}
            </p>
          </div>
        </div>
      ) : null}
      ...
      {tiktokEmbedUrl ? (
        ...
        <div className="rounded-[28px] p-8 text-center" style={cardStyle}>
          ...
          <a
            href={application.tiktok_video_url}
            target="_blank"
...
export default BusinessDealSubmissionDetail;
