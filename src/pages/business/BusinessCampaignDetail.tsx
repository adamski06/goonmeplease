...
import { ArrowLeft, Eye, Upload, ImagePlus, Trash2, Share2, Code, Copy, Check } from 'lucide-react';
...
const BusinessCampaignDetail: React.FC = () => {
  ...
  const handleThumbnailUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const path = `${user.id}/thumbnails/${id}.${ext}`;
  ...
  const handleThumbnailCropSave = async (blob: Blob) => {
    ...
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) { setUploading(false); return; }
    const path = `${user.id}/thumbnails/${id}.jpg`;
    const { error: uploadError } = await supabase.storage
      .from('campaign-assets')
