...
import { ArrowLeft, Users, CheckCircle, XCircle, Eye, ImagePlus, Trash2, Share2, Copy, Check, Code } from 'lucide-react';
...
const BusinessDealDetail: React.FC = () => {
  ...
  const handleThumbnailCropSave = async (blob: Blob) => {
    ...
    const path = `${user.id}/thumbnails/deal-${id}.jpg`;
              ...
              >
                ...
                <div className="mb-3">
                  ...
                  <div className="flex gap-2">
                    <input
                      readOnly
                      value={`${window.location.origin}/ad/${deal.id}`}
                      className="flex-1 bg-muted rounded-lg px-3 py-2 text-xs text-foreground border border-border font-mono"
                    />
                    <button
                      onClick={() => {
                        navigator.clipboard.writeText(`${window.location.origin}/ad/${deal.id}`);
                        setCopied('link');
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <input
                      readOnly
                      value={`<iframe src="${window.location.origin}/embed/ad/${deal.id}" width="420" height="160" frameborder="0" style="border:none;border-radius:20px;"></iframe>`}
                      className="flex-1 bg-muted rounded-lg px-3 py-2 text-xs text-foreground border border-border font-mono truncate"
                    />
                    <button
                      onClick={() => {
                        navigator.clipboard.writeText(`<iframe src="${window.location.origin}/embed/ad/${deal.id}" width="420" height="160" frameborder="0" style="border:none;border-radius:20px;"></iframe>`);
                        setCopied('embed');
                        setTimeout(() => setCopied(null), 2000);
                      }}
