            ...
            <form onSubmit={handleSubmit} className="h-full flex flex-col pb-8">
              ...
              {!(selectedPlatform && (selectedRegions.length > 0 || selectedAudiences.length > 0) && formData.total_budget >= 10000) ? (
                <div className="flex-1 flex flex-col items-center justify-center">
                  ...
                  <div className="w-full max-w-xs border border-input rounded-md p-6 space-y-6">
                    <button
                      type="button"
                      onClick={() => setPlatformDialogOpen(true)}
                      className="w-full h-10 px-3 text-center text-sm rounded-md border border-input bg-background text-foreground hover:bg-accent transition-colors"
                    >
                      {selectedPlatform 
