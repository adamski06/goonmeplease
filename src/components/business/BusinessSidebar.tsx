...
const BusinessSidebar: React.FC = () => {
  ...
  return (
    <aside
      className={cn(
        'border-r border-border bg-sidebar-background flex flex-col h-screen shrink-0 sticky top-0 transition-all duration-300',
        isCreationRoute ? 'w-14' : 'w-60'
      )}
    >
      {/* Logo */}
      ...
      <nav className="flex-1 px-2 py-4 overflow-y-auto space-y-1.5 min-h-0">
        ...
        <button
          ...
          className={cn(
            ...
            isCreationRoute ? 'justify-center' : '',
        ...
        <button
          ...
          className={cn(
            ...
            isCreationRoute ? 'justify-center px-0 py-2.5' : 'px-2.5 py-2'
          )}
          style={{
        ...
        {/* All ads */}
        {!isCreationRoute && items.length > 0 && (
