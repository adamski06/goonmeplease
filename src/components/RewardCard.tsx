  ...
  return (
    <div ref={cardRef} className="h-[calc(100dvh-80px)] relative flex flex-col items-center justify-start snap-start snap-always overflow-hidden">
      ...
      <div className="absolute top-10 left-0 right-0 bottom-0 pointer-events-none transition-opacity duration-700" style={{ opacity: glowVisible ? 0.4 : 0 }}>
        <img src={reward.image || placeholderBlue} alt="" className="w-full h-full object-cover scale-[1.3] blur-[55px] saturate-[1.4]" decoding="async" />
      </div>
      {/* Card image */}
      <div className="absolute top-14 left-3 right-3 bottom-3" style={getCardSlideStyle()}>
        ...
        >
          <img src={reward.image || placeholderBlue} alt={reward.brand} className="w-full h-full object-cover pointer-events-none select-none" style={{ WebkitTouchCallout: 'none', userSelect: 'none', WebkitUserSelect: 'none' } as React.CSSProperties} fetchPriority="high" decoding="async" draggable={false} />
