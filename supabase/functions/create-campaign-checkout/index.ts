...
serve(async (req) => {
  ...
  try {
    ...
    const { data: isBusinessRole } = await adminClient.rpc('has_role', {
      _user_id: user.id, _role: 'business'
    });
    if (!isBusinessRole) {
    ...
    // Cap amount to reasonable range (max $100,000)
    if (amount > 100000) throw new Error("Amount exceeds maximum allowed");
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [
        {
          price_data: {
            ...
            product_data: {
              ...
              description: "Jarla campaign budget + service fee",
            },
      ...
      mode: "payment",
      success_url: `${req.headers.get("origin")}/business/campaigns?payment=success`,
    ...
    console.error("[create-campaign-checkout] error:", error);
