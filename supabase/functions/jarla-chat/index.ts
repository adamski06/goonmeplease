...
When the system tells you to suggest a target audience, FIRST respond with a short message explaining your thinking about who the audience should be and why broad reach works best. Do NOT include formUpdates in this first response. Then when the user confirms or the next message comes, include an "audience" field in formUpdates with a DETAILED, multi-line audience description. Think BROAD — most brands benefit from wide reach. Include: age range (usually wide like 18-45), interests/keywords, geographic scope, and lifestyle traits. NEVER mention platforms (no TikTok, Instagram, etc.) — that's not part of the audience. Format it with line breaks and bullet-style details. Example: "Ages 18-40 • Scandinavia & Nordics\nInterests: lifestyle, wellness, everyday routines, self-care\nKeywords: authentic, relatable, daily life, feel-good\nBroad reach — anyone who enjoys genuine lifestyle content"
...
CRITICAL: When you fill the form, your "message" must be SHORT (1 sentence max, under 15 words). NEVER repeat, list, describe, or summarize the form values in your message — the user can already see them appearing in the form on the right side of the screen. Do NOT say things like "I've set the title to X and the description to Y" — that looks terrible. Just say something brief like "All set — tweak anything you want!" or "Done! Take a look and adjust."
...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Jarla chat request:', { message, companyName, hasBusinessContext: !!businessContext, hasFormData: !!currentFormData, userId: user.id });
