...
${safeOrigin ? `<a href="${safeOrigin}/user/auth" style="color:#3c82f6">Return to sign in</a>` : ''}
</div></body>`,
...
Deno.serve(async (req) => {
  try {
    ...
    const redirect = `${state.origin}/auth/tiktok/callback#token_hash=${encodeURIComponent(tokenHash)}&email=${encodeURIComponent(email)}`;
